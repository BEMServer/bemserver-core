...
class TestTimeseriesDataModel:
    ...
    def test_timeseries_data_authorizations_as_admin(
        ...
        with CurrentUser(admin_user):
            ...
            with CurrentCampaign(campaign_1):
                with pytest.raises(BEMServerAuthorizationError):
                    TimeseriesData.check_can_export(start_dt, end_dt_out, [ts_1.id])
            ...
            with CurrentCampaign(campaign_1):
                with pytest.raises(BEMServerAuthorizationError):
                    TimeseriesData.check_can_import(start_dt, end_dt_out, [ts_1.id])

    @pytest.mark.usefixtures("users_by_campaigns")
    @pytest.mark.usefixtures("timeseries_by_campaigns")
    def test_timeseries_data_authorizations_as_user(self, users, campaigns, timeseries):
        user_1 = users[1]
        ...
        with CurrentUser(user_1):
            ...
            with CurrentCampaign(campaign_2):
                with pytest.raises(BEMServerAuthorizationError):
                    TimeseriesData.check_can_export(start_dt, end_dt_out, [ts_2.id])
            ...
            with CurrentCampaign(campaign_2):
                with pytest.raises(BEMServerAuthorizationError):
                    TimeseriesData.check_can_import(start_dt, end_dt_out, [ts_2.id])
