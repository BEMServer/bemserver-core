...
class TestOutliersProcess:
    @pytest.mark.parametrize("timeseries", (4,), indirect=True)
    def test_outliers_process(self, users, timeseries):
        admin_user = users[0]
        ...
        with CurrentUser(admin_user):
            ...
            ret = get_outliers(start_dt, end_dt, ts_l, ds_1, inclusive="both")
            expected = pd.DataFrame(
                {
                    ts_0.id: [0, np.nan, np.nan, np.nan, 69],
                    ts_1.id: [0, np.nan, np.nan, np.nan, np.nan],
                    ts_2.id: [np.nan, np.nan, np.nan, np.nan, np.nan],
