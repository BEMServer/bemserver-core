...
class TestEnergyPowerProcess:
        ...
        with CurrentUser(admin_user):
            ...
            timestamps = sum(
                [
                    [
                        ...
                        dt.datetime(2020, 1, 1, hour, 30, tzinfo=dt.UTC),
                    ]
                    for hour in (0, 1, 2, 3)
                ],
                [],
            ...
            expected_data_s = pd.Series(
                ...
                index=pd.DatetimeIndex(
                    timestamps, name="timestamp", freq="1800s"
                ).as_unit("us"),
            )
            ...
            timestamps = [
                dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC) for hour in (0, 1, 2, 3)
            ]
            expected_data_s = pd.Series(
                [0.0, 1.0, 0.0, 1.0],
                index=pd.DatetimeIndex(
                    timestamps, name="timestamp", freq="3600s"
                ).as_unit("us"),
            )
            assert_series_equal(data_s, expected_data_s)
            ...
            timestamps = [
                dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC) for hour in (0, 2)
            ]
            expected_data_s = pd.Series(
                [1.0, 1.0],
                index=pd.DatetimeIndex(
                    timestamps, name="timestamp", freq="7200s"
                ).as_unit("us"),
            )
            assert_series_equal(data_s, expected_data_s)
            ...
            timestamps = [
                dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC) for hour in (0, 4, 8)
            ]
            expected_data_s = pd.Series(
                [np.nan, 168000.0, 168000.0],
                index=pd.DatetimeIndex(
                    timestamps, name="timestamp", freq="14400s"
                ).as_unit("us"),
            )
            assert_series_equal(data_s, expected_data_s)
            ...
            timestamps = sum(
                [
                    [
                        dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC).astimezone(
                            ZoneInfo("Europe/Paris")
                        ),
                        dt.datetime(2020, 1, 1, hour, 30, tzinfo=dt.UTC).astimezone(
                            ZoneInfo("Europe/Paris")
                        ),
                    ]
                    for hour in (0, 1, 2, 3)
    ...
    def test_energy2power_process(self, users, timeseries):
        ...
        with CurrentUser(admin_user):
            ...
            timestamps = [
                dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC) for hour in (6, 9)
            ]
            expected_data_s = pd.Series(
                [6.0, 9.0],
                index=pd.DatetimeIndex(timestamps, name="timestamp").as_unit("us"),
            ...
            timestamps = [dt.datetime(2020, 1, 1, 6, 0, tzinfo=dt.UTC)]
            expected_data_s = pd.Series(
                [
                    21000.0,
                ],
            ...
            expected_data_s = pd.Series(
                [],
                index=pd.DatetimeIndex(timestamps, name="timestamp", tz=dt.UTC).as_unit(
                    "us"
                ),
                dtype=float,
            )
    ...
    def test_energyindex2power_process(self, users, timeseries):
        ...
        with CurrentUser(admin_user):
            ...
            expected_data_s = pd.Series(
                ...
                index=pd.DatetimeIndex(timestamps, name="timestamp", freq="h").as_unit(
                    "us"
                ),
            )
            assert_series_equal(data_s, expected_data_s)
            ...
            timestamps = [
                dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC) for hour in (0, 2, 4)
            ]
            expected_data_s = pd.Series(
                [2.0, 6.0, 9.0],
                index=pd.DatetimeIndex(timestamps, name="timestamp", freq="2h").as_unit(
                    "us"
                ),
            )
            assert_series_equal(data_s, expected_data_s)
            ...
            timestamps = [
                dt.datetime(2020, 1, 1, hour, 0, tzinfo=dt.UTC) for hour in range(12)
            ]
            expected_data_s = pd.Series(
                12 * [np.nan],
                index=pd.DatetimeIndex(timestamps, name="timestamp", freq="h").as_unit(
            ...
            expected_data_s = pd.Series(
                3 * [np.nan],
                index=pd.DatetimeIndex(
                    timestamps, name="timestamp", tz=dt.UTC, freq="4h"
                ).as_unit("us"),
                dtype=float,
            )
            assert_series_equal(data_s, expected_data_s)
