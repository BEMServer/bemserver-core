...
class CheckMissingDataBase(BEMServerCoreTask):
    DEFAULT_PARAMETERS = {
        "min_completeness_ratio": 0.9,
    }

    def do_run(self, campaign, start_dt, end_dt, min_completeness_ratio=0.9):
        ...
        nb_c_scopes = len(campaign.campaign_scopes)

        for idx, c_scope in enumerate(campaign.campaign_scopes):
            ...
            for ts in c_scope.timeseries:
                ...
                # TS is missing if
                # either count/expected < min
                # or no expectation and count=0
            ...
            self.set_progress(idx + 1, nb_c_scopes)
...
class CheckMissingData(CheckMissingDataBase, BEMServerCoreAsyncTask):
    """CheckMissingData async task"""
