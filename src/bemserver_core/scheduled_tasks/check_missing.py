...
CHECK_MISSING_TASK_NAME = "CheckMissing"
...
@celery.task(name=CHECK_MISSING_TASK_NAME)
def check_missing_scheduled_task():
    logger.info("Start")

    for tbc in TaskByCampaign.get(name=CHECK_MISSING_TASK_NAME, is_enabled=True):
        start_dt, end_dt = TaskByCampaign.make_interval(tbc.campaign, tbc.parameters)
        check_missing_ts_data(
            tbc.campaign,
            start_dt,
            end_dt,
            min_completeness_ratio=tbc.params.get("min_completeness_ratio", 0.9),
        )
